...
import { ArrowLeft, FileText, Download, CheckCircle2, XCircle, Sparkles } from 'lucide-react'
...
function generateSummary(
  ...
  if (total === 0) {
    return `No required skills were defined for this job, so the AI score is based on resume quality alone. The candidate's resume contains ${additional.length} extracted skill${additional.length === 1 ? '' : 's'}.`
  }
...
export default function ApplicationDetailPage() {
  ...
  // Fetch full job detail so we get required skills for the breakdown
  ...
  // Compute breakdown once job skills are available
  const breakdown =
    jobDetail && application.extracted_skills
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {breakdown ? (
              ...
              <div className="space-y-4">
                  ...
                  {breakdown.matched.length === 0 ? (
                    ...
                    <div className="flex flex-wrap gap-2 pl-6">
                      {breakdown.matched.map((s) => (
                        <span
                          key={s.id}
                          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800 border border-emerald-200"
                        >
                ...
                <div>
                  ...
                  {breakdown.missing.length === 0 ? (
                    <p className="text-xs text-emerald-600 pl-6 font-medium">
            ...
            // Fallback: job detail still loading or no required skills defined
            <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          {application.ai_log && (
            <div className="bg-gray-900 rounded-xl p-6 text-gray-100">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs font-mono text-gray-400">
                <p>
                  Tokens used:{' '}
                  <span className="text-gray-200">{application.ai_log.tokens_used}</span>
                </p>
                <p>
                  Processing time:{' '}
                  <span className="text-gray-200">{application.ai_log.processing_time_ms}ms</span>
                </p>
            ...
            {application.status_history.length === 0 ? (
              ...
              <div className="space-y-4">
                {[...application.status_history]
                  .sort(
                    (a, b) =>
                      new Date(b.date_changed).getTime() -
                      new Date(a.date_changed).getTime()
                  )
                  .map((entry, i) => (
