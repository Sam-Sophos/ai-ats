...
export interface StatusHistory { id: number; status: Status; changed_by: User; date_changed: string }
...
export interface ApplicationDetail { id: number; candidate: Candidate; job: JobList; ai_match_score: number | null; applied_date: string; resume_file: string; extracted_skills: Skill[]; current_status: Status | null; status_history: StatusHistory[]; offer: JobOffer | null; ai_log: AIProcessingLog | null; task_id?: string; matched_skills: Skill[]; missing_skills: Skill[]; additional_skills: Skill[]; ai_breakdown_summary: string; years_experience: number | null; education_level: string | null }
